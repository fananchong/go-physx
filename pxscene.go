...
func (this *PxScene) CreatePlane(yAxis float32) uint64 {
	return uint64(C.CreatePlane(this.c, C.float(yAxis)))
}

func (this *PxScene) CreateBoxDynamic(pos Vector3, halfExtents Vector3) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	cHalfExtents := C.Vector3{}
	cHalfExtents.X = C.float(cHalfExtents.X)
	cHalfExtents.Y = C.float(cHalfExtents.Y)
	cHalfExtents.Z = C.float(cHalfExtents.Z)
	return uint64(C.CreateBoxDynamic(this.c, &cPos, &cHalfExtents))
}

func (this *PxScene) CreateBoxKinematic(pos Vector3, halfExtents Vector3) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	...
	cHalfExtents.Y = C.float(cHalfExtents.Y)
	cHalfExtents.Z = C.float(cHalfExtents.Z)
	return uint64(C.CreateBoxKinematic(this.c, &cPos, &cHalfExtents))
}

func (this *PxScene) CreateBoxStatic(pos Vector3, halfExtents Vector3) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	...
	cHalfExtents.Y = C.float(cHalfExtents.Y)
	cHalfExtents.Z = C.float(cHalfExtents.Z)
	return uint64(C.CreateBoxStatic(this.c, &cPos, &cHalfExtents))
}

func (this *PxScene) CreateSphereDynamic(pos Vector3, radius float32) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	return uint64(C.CreateSphereDynamic(this.c, &cPos, C.float(radius)))
}

func (this *PxScene) CreateSphereKinematic(pos Vector3, radius float32) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	return uint64(C.CreateSphereKinematic(this.c, &cPos, C.float(radius)))
}

func (this *PxScene) CreateSphereStatic(pos Vector3, radius float32) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	return uint64(C.CreateSphereStatic(this.c, &cPos, C.float(radius)))
}

func (this *PxScene) CreateCapsuleDynamic(pos Vector3, radius, halfHeight float32) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	return uint64(C.CreateCapsuleDynamic(this.c, &cPos, C.float(radius), C.float(halfHeight)))
}

func (this *PxScene) CreateCapsuleKinematic(pos Vector3, radius, halfHeight float32) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	return uint64(C.CreateCapsuleKinematic(this.c, &cPos, C.float(radius), C.float(halfHeight)))
}

func (this *PxScene) CreateCapsuleStatic(pos Vector3, radius, halfHeight float32) uint64 {
	cPos := C.Vector3{}
	cPos.X = C.float(cPos.X)
	cPos.Y = C.float(cPos.Y)
	cPos.Z = C.float(cPos.Z)
	return uint64(C.CreateCapsuleStatic(this.c, &cPos, C.float(radius), C.float(halfHeight)))
}
